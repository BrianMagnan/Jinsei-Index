...
export const getCategories = async (req, res) => {
  try {
    const categories = await Category.find({ profile: req.profileId }).sort({ name: 1 });
    res.json(categories);
  } catch (error) {
...
export const getCategory = async (req, res) => {
  try {
    const category = await Category.findOne({ _id: req.params.id, profile: req.profileId });
    ...
    const skills = await Skill.find({ category: category._id, profile: req.profileId });
...
export const createCategory = async (req, res) => {
  try {
    ...
    const category = new Category({ name, description, profile: req.profileId });
    await category.save();
